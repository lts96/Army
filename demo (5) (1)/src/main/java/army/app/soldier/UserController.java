...
import army.domain.model.LoginOutput;
...
public class UserController {
    ...
    @ResponseBody
    @RequestMapping(value = "/main/sign_in",method = RequestMethod.POST)  // 유저 정보 변경 + 회원가입 , 유저 이름이 다르면 새로 등록되고 같으면 갱신된다   왜지????
    public ResponseEntity<?> changeUserInfo(@RequestBody User user ) {    //user 객체 포맷이랑 정확하게 맞춰서 input 넣어야됨. 
    	List<User> users = userService.findAllUser();
    	for(int i=0;i<users.size();i++) {
    		if(users.get(i).getUserId().equals(user.getUserId()))
    		{
    			return new ResponseEntity<>("REGISTER_FAIL_DUPID", HttpStatus.OK);
    		}
    	}
    	userService.save(user);
    	return new ResponseEntity<>("REGISTER_OK", HttpStatus.OK);
    }
    
    
    @ResponseBody
    @RequestMapping(value = "/main/log_in",method = RequestMethod.PUT)   																			
    public ResponseEntity<LoginOutput> login(@RequestBody LoginForm login ) {
    	List<User> users = userService.findAllUser();
    	LoginOutput result = new LoginOutput();
    	for(int i=0;i<users.size();i++)
    	{
    		if(users.get(i).getUserId().equals(login.getId()))
    		{
    			if(users.get(i).getPassword().equals(login.getPw()))
        		{
    				result.setResult("LOGIN_SUCCESS"); 
        			return new ResponseEntity<LoginOutput>(result, HttpStatus.OK);
        		}
    			else 
    			{
    		...
    		//System.out.println("user pw : "+ users.get(i).getPassword());
    		//System.out.println("input id : "+login.getId());
			//System.out.println("input pw : "+login.getPw());
    	}
    ...
    */
    ...
    public String userGroupList(@RequestBody String user_id)  // 지금은 유저당 그룹 하나를 관리한다고 생각하고 만듬 
    {																					  // 현재 관리하는 그룹 이름만 보냄   okok
    	...
    	for(int i=0;i<soldierGroups.size();i++)
    	{
    		...
    		{
    			userGroup.add(soldierGroups.get(i));
    			name = soldierGroups.get(i).getGroupName(); 
            	return name;
    		}
    	}
    	return "EMPTY";
	...
	public ResponseEntity<List<Soldier>> userSoldierList(@RequestBody String user_id)
	{
		User user = userService.findUser(user_id);
    	List<SoldierGroup> soldierGroups = soldierService.findAllGroups();
    	List<SoldierGroup> userGroup = new ArrayList<SoldierGroup>();
