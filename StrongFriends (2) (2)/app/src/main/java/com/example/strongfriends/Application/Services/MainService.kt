...
class MainService :Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        var bitmap=BitmapFactory.decodeResource(this.resources, R.mipmap.ic_launcher)
        startForeground(1,NotificationFactory.create(this,"강한친구육군","메인서비스", R.mipmap.ic_launcher))
        var timer= Timer()
            ...
            apiService.periodicQuery(Periodic_body(Option.user_id,Option.lastUpdate.toString()))
                ...
                .subscribe({it->
                    ...
                    if(it.isLastOption==0){ //최신 옵션이라면?
                        ...
                        mIsBound=bindService(Intent(applicationContext,ControlService::class.java),mConnection, Context.BIND_AUTO_CREATE) //서비스를 바인드한다.
                        Log.d("hsh","MainService 쿼리에 대한 답장으로 새 옵션이 도착.")
                        setOption(it)
    ...
    fun setOption(option:Periodic_response){
        ...
        Option.timer="2019052813000020190528141100"
    ...
    companion object {
        val mActivityMessenger: Messenger = Messenger(myHandler()) //메신전데 내가 만든 핸들러를 담은 거야. 내가 밑에서 메세지 보낼거지? 거기에 대한 답은 여기에 하라고.
