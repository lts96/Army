...
class LockService: Service() {
    companion object{
        var broadcast:LockBroadcastReceiver=LockBroadcastReceiver()
        var isStart=0
        var isBroad=0
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int { //startService에서 여기로 넘어온다.
        ...
        if(isStart==0) //한번 실행되었던 놈이면 startForeground를 재실행 시키지는 않는다.
            startForeground(1, NotificationFactory.create(applicationContext,"스크린","${Option.getFormattedTime()[0].toLocalTime()}에 알람이 시작됩니다", R.drawable.notification_template_icon_bg))
        isStart=1
        if(broadcast==null){
            ScreenLock()
        }
