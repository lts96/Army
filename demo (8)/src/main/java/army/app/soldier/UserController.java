...
//@CrossOrigin(origins = {"http://221.155.56.120:8100","http://221.155.56.120:8080" })
...
public class UserController {
    ...
    //  텅빈 그룹 하나 만들어주기, 유저는 이 group의 name (핀 번호)를 가지게 된다. 생성된 빈 그룹을 리턴 
    
	@ResponseBody
    @RequestMapping(value = "/main/userlist/{userId}/createGroup",method = RequestMethod.POST)  // 생성은 되는데 빈값 들어감
    public ResponseEntity<?> createEmptyGroup(@PathVariable("userId")String userId ,@RequestBody Soldier soldier ) {
	...
	@ResponseBody
	@RequestMapping(value = "/main/vacation/{userId}/{soldierId}",method = RequestMethod.PUT)
